...
import '../bar/SideEffects';
import { WindowLayer } from 'lib/types/options.js';
import { Attribute, Child } from 'lib/types/widget.js';
import Window from 'types/widgets/window.js';

const { layouts } = options.taskbar;
const { location } = options.theme.taskbar;

export type TaskbarWidget = keyof typeof widget;
                        ...
                        setup: (self) => {
                            self.hook(layouts, (self) => {
                                ...
                                self.children = foundLayout.left
                                    ...
                                    .reduce((res, w) => {
                                        const child = widget[w](hyprlandMonitor);
                                        if (child.boxClass === 'windowlist') {
                                            console.log('windowlist found!!!');
                                            child.component.children.forEach((sub) =>
                                                res.push(sub as Button<Gtk.Widget, unknown>),
                                            );
                                        } else {
                                            res.push(child as Button<Gtk.Widget, unknown>);
                                        }
                                        return res;
                                    }, []);
                            });
